...
public class EnumerableExtensionTests
{
    ...
    {
        var (output, tenet) = rank.Unwind(source);
    ...
    {
        var rank = Enumerable.Range(0, 100).Select(i => (double)i).ToList();
        var source = rank.Select(r => Random.Shared.Next()).ToArray();
        Random.Shared.Shuffle(source);
        
        var (trans, tenet) = rank.Unwind(source);
        var (output, outputTenet) = tenet.Unwind(trans);
